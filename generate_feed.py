...
def save_current_isbns(isbns_list):
    ...
    try:
        ...
        with open(PREVIOUS_ISBNS_FILE, 'w', encoding='utf-8') as f:
            json.dump(isbns_to_save, f, indent=2)
...
def get_all_isbns():
    ...
    try:
        response = requests.get(OPENBD_API_COVERAGE_URL, timeout=90)
...
def get_book_details(isbn_list):
    ...
    try:
        ...
        response = requests.post(OPENBD_API_GET_URL, data={'isbn': isbn_string}, timeout=180)
        response.raise_for_status()
        data = response.json()
...
def parse_pubdate(pubdate_str):
    ...
    dt = None
    formats_to_try = [
        ("%Y%m%d", False), ("%Y-%m-%d", False), ("%Y年%m月%d日", False), ("%Y", True),
    ]
    for fmt, is_year_only in formats_to_try:
        try:
            dt = datetime.strptime(pubdate_str, fmt)
            if is_year_only: dt = dt.replace(month=1, day=1)
            break
                ...
                if text_content_list and isinstance(text_content_list, list):
                    for content in text_content_list:
                        if isinstance(content, dict) and 'TextType' in content and 'Text' in content:
                            text_type, text = content.get('TextType'), content.get('Text', '')
                            if text_type == '03': description = text; break
                            elif text_type == '02': toc = text
    except Exception as e: logging.warning(f"Error extracting description: {e}")
    final_description = description if description else toc
    # if final_description: final_description = re.sub('<[^<]+?>', '', final_description)
    return final_description

def filter_books(book_data_list):
    """Filters books based on target C-Codes ('78' scheme) and valid, parsable pubdate."""
    filtered_list = []
    processed_count, matched_count = 0, 0
    ...
    for book_data in book_data_list:
        ...
        if not book_data or not isinstance(book_data, dict) or 'summary' not in book_data:
            logging.debug(f"Item {processed_count}: Skipping invalid book data structure."); continue
        summary = book_data.get('summary')
        if not isinstance(summary, dict): logging.debug(f"Item {processed_count}: Skipping due to non-dict summary."); continue
        isbn = summary.get('isbn')
        title = summary.get('title', '[No Title]')
        if not isbn or not summary.get('title'):
            logging.debug(f"Item {processed_count} (ISBN: {isbn or 'N/A'}): Skipping due to missing ISBN or title."); continue

        c_code_match, c_code_found = False, None
        try:
                 ...
                 if isinstance(descriptive_detail, dict):
                     ...
                     if isinstance(subjects, list):
                         for subject in subjects:
                             if isinstance(subject, dict) and subject.get('SubjectSchemeIdentifier') == '78':
                                 c_code_found = subject.get('SubjectCode')
                                 if c_code_found in TARGET_C_CODES: c_code_match = True; break
        except Exception as e: logging.warning(f"ISBN {isbn} ('{title}'): Error accessing subject data: {e}. Assuming no C-Code match.")

        if c_code_match:
            pubdate_str = summary.get('pubdate')
            parsed_date = parse_pubdate(pubdate_str)
            if parsed_date:
                filtered_list.append((book_data, parsed_date)); matched_count += 1
                logging.debug(f"ISBN {isbn} ('{title}'): MATCHED! (C-Code: {c_code_found}, Pubdate: {parsed_date})")
            else:
                log_msg = f"ISBN {isbn} ('{title}'): Skipping. Matched C-Code '{c_code_found}' but "
                log_msg += f"pubdate '{pubdate_str}' is unparsable/invalid." if pubdate_str else "pubdate is missing."
                logging.debug(log_msg)
        elif logging.getLogger().isEnabledFor(logging.DEBUG):
            log_msg = f"ISBN {isbn} ('{title}'): Skipping because "
            log_msg += "no C-Code with Scheme '78' was found." if c_code_found is None else f"found C-Code '{c_code_found}' (Scheme 78) is not in TARGET_C_CODES."
            logging.debug(log_msg)
...
def generate_rss_feed(filtered_books):
    ...
    fg = FeedGenerator()
    feed_link_url = "https://analekt.github.io/"
    ...
    if books_for_feed: latest_pubdate = books_for_feed[0][1]; fg.pubDate(latest_pubdate)
    else: fg.pubDate(now_jst)

    for book_data, parsed_pubdate in books_for_feed:
        summary = book_data.get('summary', {}); onix = book_data.get('onix', {})
        isbn = summary.get('isbn'); title = summary.get('title'); author = summary.get('author', '')
        if not isbn or not title: continue

        fe = fg.add_entry()
        fe.title(title); fe.id(isbn)
        fe.link(href=f"{FEED_LINK_BASE}{isbn}", rel='alternate')
        if author: fe.author({'name': author})
        fe.description(get_description(onix)); fe.pubDate(parsed_pubdate)

    try:
        fg.rss_file(OUTPUT_FILE, pretty=True, encoding='utf-8')
        logging.info(f"Successfully wrote RSS feed to {OUTPUT_FILE}")
        if not os.path.exists(OUTPUT_FILE): logging.warning(f"Despite success log, {OUTPUT_FILE} was not found on disk.")
        elif os.path.getsize(OUTPUT_FILE) == 0: logging.warning(f"{OUTPUT_FILE} was created but is empty.")
    except Exception as e: logging.error(f"Error writing RSS feed file {OUTPUT_FILE}: {e}")
...
if __name__ == "__main__":
    ...
    if not current_isbn_list: logging.error("Failed to retrieve current ISBN list. Exiting."); exit(1)
    current_isbn_set = set(current_isbn_list)

    if not previous_isbn_set:
        logging.info("First run detected. Saving current list and generating initial empty feed.")
        save_current_isbns(current_isbn_list)
        generate_rss_feed([])
        logging.info("Initial ISBN list saved. Exiting first run.")
    else:
        ...
        if removed_isbn_set: logging.info(f"Detected {len(removed_isbn_set)} ISBNs removed from coverage.")
        ...
        if new_isbn_set:
            ...
            for i in range(0, len(new_isbn_list), CHUNK_SIZE):
                ...
                if details: new_book_data.extend(details); total_fetched_details += len(details)
                if i + CHUNK_SIZE < len(new_isbn_list): time.sleep(REQUEST_DELAY)
            logging.info(f"Retrieved details for {total_fetched_details} out of {len(new_isbn_list)} new ISBNs.")
        else: logging.info("No new ISBNs found since last run.")

        if new_book_data: filtered_new_books = filter_books(new_book_data)
        else: filtered_new_books = []; logging.info("No new book details to filter.")

        generate_rss_feed(filtered_new_books)
        save_current_isbns(current_isbn_list) # Save latest list after successful subsequent run
